...
void PositionManager::enableGPS(bool enable) {
    // Il GPS è abilitato/disabilitato in base alla configurazione in config.h
    (void)enable;  // Evita warning per parametro non utilizzato
    #ifdef ENABLE_GPS
    if (enable) {
        _gps = &gps;  // Assumendo che esista un'istanza globale di GPS
    } else {
        _gps = nullptr;
    }
    #else
...
void PositionManager::moveStraight(float distance, float speed) {
    if (_leftMotor && _rightMotor) {
        ...
        _leftMotor->setPowerPercent(static_cast<int8_t>(speed * 100));
        _rightMotor->setPowerPercent(static_cast<int8_t>(speed * 100));
        
        // Attendi il tempo necessario
        delay(static_cast<unsigned long>(time * 1000));
        
        // Ferma i motori
        _leftMotor->setPowerPercent(0);
        _rightMotor->setPowerPercent(0);
    }
}
...
void PositionManager::moveArc(float radius, float angle, float speed) {
    if (_leftMotor && _rightMotor) {
        // Calcola la lunghezza dell'arco e il tempo necessario
        float distance = radius * abs(angle);  // Lunghezza dell'arco in metri
        float time = distance / speed;         // Tempo in secondi
        
        // Calcola le velocità delle ruote per la traiettoria circolare
        float leftSpeed = speed * ((radius + (WHEEL_BASE / 2.0f)) / radius);
        float rightSpeed = speed * ((radius - (WHEEL_BASE / 2.0f)) / radius);
        
        // Imposta la velocità dei motori in percentuale
        _leftMotor->setPowerPercent(static_cast<int8_t>(leftSpeed * 100));
        _rightMotor->setPowerPercent(static_cast<int8_t>(rightSpeed * 100));
